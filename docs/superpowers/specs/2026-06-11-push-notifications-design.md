...
- Mobile app: Expo 54, RN 0.81, React Navigation (native-stack + bottom-tabs, **not**
  expo-router). iOS bundle `com.kupapay.mobile` (Apple Team `K3M6R85KA6`), Android
  `com.kupapay.mobile`, EAS owner `saussilberg`. Deep links already configured for
...
2. Firebase: create/confirm an FCM project for `com.kupapay.mobile` and provide credentials to
   EAS.
