# Handoff: KupaPay · Group Detail Screen

## Overview

This is the **group detail screen** for the KupaPay cost-sharing app — the screen a user sees after tapping a group on the list. It shows the group's cover image, the user's balance in this group, a quick "Settle up" action, a "Note" affordance, and the activity feed (expenses + chat messages). A floating action pair lets the user add an expense or send a message.
